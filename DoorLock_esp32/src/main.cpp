...
bool Password = true; // the servo 
// the pins to plug the keypad into
static byte rowPins[ROWS] = {19, 18, 5, 17};
static byte colPins[COLS] = {16, 4, 2, 15};
...
void Open() // function declarations
{
  ...
  if (Particular_Count == Password_Length - 1) // gets the length of the password 
  {
    ...
    else
    {
      ...
      delay(1000);
      lcd.clear();
      Password = 1; // closes the smart door
    }
...
void loop()
{
  if (Password == 0) // opens the smart door
  {
